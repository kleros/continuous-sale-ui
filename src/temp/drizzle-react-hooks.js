...
export const DrizzleProvider = ({ children, drizzle }) => {
  const useCacheCall = useCallback((
    ...
    const drizzleState = useDrizzleState(drizzleState => {
      if (isFunction)
        ...
        const cacheKey = drizzle.contracts[contractNameOrContractNames].methods[
          methodNameOrFunction
        ].cacheCall(...args)
        return {
          value:
  ...
  const useCacheSend = useCallback((contractName, methodName) => {
    const drizzleState = useDrizzleState(drizzleState => ({
    ...
    return {
      send: (...args) =>
        setStackIDs(stackIDs => [
          ...stackIDs,
          drizzle.contracts[contractName].methods[methodName].cacheSend(...args)
        ]),
      status:
